...
*COPY TRACEWK SUPPRESS.
 ...
 01  WK-CHEDXD8         PIC X(11) VALUE SPACES.
 01  BEGINTIME.
...
* 檢驗執行週 (W) 報表或月 (M) 報表
     ...
     IF BEGINDATE = SPACES AND ENDDATE = SPACES
         IF TYPE-RPT = 'W' AND WK-DD NOT = 01
             ...
             MOVE '00960209' TO BEGINTIME ENDTIME
             MOVE 'Y'        TO DATE-SW
         ELSE
             IF TYPE-RPT = 'M'
                 IF WK-DD < 20
         ...
         END-IF
     ELSE
         COMPUTE BEGINDATE = BEGINDATE - 19110000
         COMPUTE ENDDATE   = ENDDATE   - 19110000
         MOVE BEGINDATE    TO BEGINTIME
         MOVE ENDDATE      TO ENDTIME
         MOVE 'Y'          TO DATE-SW
     END-IF.
...
*******************************************************
     ...
     IF PSTATUS OF CHPCB = SPACES OR 'FW'
         ADD 1               TO CHIL-CNT
         MOVE 0              TO WK-DATE
         MOVE CHEDXD         TO WK-DATE(3:6)
         IF CHEDXD IS NUMERIC
             MOVE CHEDXD     TO WK-DATE(3:6)
         ELSE
             MOVE SPACES     TO WK-CHEDXD8
             MOVE CHEDXD8    TO WK-CHEDXD8
             MOVE WK-CHEDXD8(4:8) TO WK-DATE
         END-IF
         IF WK-DATE NOT < BEGINTIME AND WK-DATE NOT > ENDTIME
             DISPLAY '#' ROOT-CNT '#' WK-CH-KEY '/' CHEDSQ
                     '/' CHESTC '/' CHECRT
             IF CHESTC = '6' OR '2' OR '3'
...
*******************************************************
     ...
     IF CHECSN = CHRCSN
         MOVE CHRCCC         TO OUT-CHRCCC
     ELSE
         IF CHECSN = CHRCSN1
             MOVE CHRCCC1    TO OUT-CHRCCC
         END-IF
     END-IF.
